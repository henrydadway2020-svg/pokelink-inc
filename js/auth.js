...
(function () {
  ...
  var DISCORD_CONFIG = Object.assign({
    ...
    messageType: 'pokelink:discord-auth',
  ...
  function discordEnabled() {
    ...
    return localOrigin || location.hostname === 'beta.pokelink.pages.dev';
  }
  ...
  // custom token y se reconoce por el claim pokelinkProvider que emite el backend.
  ...
  var DISCORD_FLOW_STORE = 'pokelink_discord_auth_v1';
  var DISCORD_FLOW_CHANNEL = 'pokelink_discord_auth_result_v1';
  ...
  function mirrorDiscordFlowToPopup(popup) {
    ...
    try {
      ...
      // en el sessionStorage propio del popup y reaparece solo si vuelve a pokelink.
  ...
  function openDiscordPopup() {
    ...
    try {
      ...
      popup = window.open('about:blank', 'pokelink-discord-auth',
        'popup=yes,width=' + w + ',height=' + h + ',left=' + left + ',top=' + top + ',resizable=yes,scrollbars=yes');
      if (popup) {
        popup.document.title = 'PokeLink';
  ...
  function exportData() {
    ...
    var data = {
      app: 'PokeLink', exportedAt: new Date().toISOString(),
    ...
    try {
      ...
      a.href = url; a.download = 'pokelink-datos.json';
